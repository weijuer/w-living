...
/**
   * `WTabs` is a container element for tabs and panels.
   *
   * All children of `<w-tabs>` should be either `<w-tab>` or
   * `<w-tabpanel>`. This element is stateless, meaning that no values are
   * cached and therefore, changes during runtime work.
   */
class WTabs extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        Promise.all([
            customElements.whenDefined('w-tab'),
            customElements.whenDefined('w-panel'),
        ])
    ...
    _linkPanels() {
        ...
        tabs.forEach(tab => {
            ...
            if (panel.tagName.toLowerCase() !== 'w-panel') {
                console.error(`Tab #${tab.id} is not a` +
                    `sibling of a <w-panel>`);
                return;
            }
     ...
     */
    _allPanels() {
        return Array.from(this.querySelectorAll('w-panel'));
    }
...
export const registerTabs = () => customElements.define('w-tabs', WTabs);
