// `wTabCounter` counts the number of `<w-tab>` instances created. The
// number is used to generated new, unique IDs.
let wTabCounter = 0;
/**
 * `WTabsTab` is a tab for a `<w-tabs>` tab panel. `<w-tab>`
 * should always be used with `role=heading` in the markup so that the
 * semantics remain useable when JavaScript is failing.
 *
 * A `<w-tab>` declares which `<w-panel>` it belongs to by
 * using that panel’s ID as the value for the `aria-controls` attribute.
 *
 * A `<w-tab>` will automatically generate a unique ID if none
 * is specified.
 */
class WTab extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (!this.id)
            this.id = `w-tab-generated-${wTabCounter++}`;
...
export const registerTab = () => customElements.define('w-tab', WTab);
